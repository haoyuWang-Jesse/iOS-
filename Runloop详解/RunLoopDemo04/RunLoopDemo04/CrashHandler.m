...
static NSUncaughtExceptionHandler *hadRegisteredHandle;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        //[self writeDataToSandboxWithData:@"hahah"];
    }
    return self;
}
...
{
    hadRegisteredHandle = NSGetUncaughtExceptionHandler();
    if(hadRegisteredHandle) {
        // 1.捕获一些异常导致的崩溃
...
- (void)handleException:(NSException *)exception
{
    ...
    if(hadRegisteredHandle && ![[exception name] isEqual:kSignalExceptionName]) {
        hadRegisteredHandle(exception); //callback other third serve
    }
...
- (void)writeDataToSandboxWithData:(NSString *)content {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *doc = [NSString stringWithFormat:@"%@/crash.txt",[paths objectAtIndex:0]];
    if (content) {
...
void SignalHandler(int signal)
{
    ...
    [crashObject  writeDataToSandboxWithData:[customException reason]];
    
    /*
    kill(getpid(), signal);
    [crashObject resetHandler];
    */
